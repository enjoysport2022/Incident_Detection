...
featureList_time=featureList_time[1:len(featureList_time)]
...
clf.fit(featureList[:-10],dummyY[:-10])

for i in range(1,11):
    prediction=clf.predict(featureList[-i])
    print(featureList_time[-i])
    print("prediction:",prediction)

    if prediction==0:
        r = requests.post("http://httpbin.org/post", data = {"mode":0,"time":featureList_time[-i],"ID":"..."})
    print("really:",dummyY[-i])
